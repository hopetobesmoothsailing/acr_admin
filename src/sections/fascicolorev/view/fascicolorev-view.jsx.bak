...
export default function FascicolorevView() {
    ...
    */
    ...
    const segmento = parseInt(searchParams.get('seg'),10) || 1;
    ...
    useEffect(() => {
        ...
        `${date.getDate().toString().padStart(2, '0')}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getFullYear()} ${date.getHours().toString().padStart(2, '0')}`;
    
        const processUserRecords = (records) => {
            records.sort((a, b) => parseDateTime(a.recorded_at) - parseDateTime(b.recorded_at));
            const segments = {};
            records.forEach((record) => {
                const dateTime = parseDateTime(record.recorded_at);
                const segmentKey = Math.floor(dateTime.getTime() / (segmento * 60 * 1000));
                if (!segments[segmentKey]) segments[segmentKey] = [];
                segments[segmentKey].push(record);
            });
        ...
        const rebuildAcrDetails = (acrDetailsx) => {
            const groupedByUser = acrDetailsx.reduce((acc, detail) => {
                const { user_id } = detail;
                acc[user_id] = acc[user_id] || [];
                acc[user_id].push(detail);
                return acc;
            }, {});
    
            let rebuiltDetails = [];
    
            Object.values(groupedByUser).forEach(userRecords => {
                const processedRecords = processUserRecords(userRecords);
                rebuiltDetails = [...rebuiltDetails, ...processedRecords];
            });
        ...
        const fetchACRDetailsByDate = async () => {
            ...
            try {
                ...
                const rebuiltDetails = rebuildAcrDetails(response.acrDetails);
                setACRDetails(rebuiltDetails);
            } catch (error) {
    ...
    }, [selectedDate,tipoRadioTV,segmento]);
    console.log("ACR_DETAILS",acrDetails );
    ...
    const generateTimeSlots = (intervalValuex) => {
        const slots = {
            "00:00 - 23:59": [],
        ...
        while (currentMinute < minutesInDay) {
            ...
            const endMinute = Math.min(currentMinute + intervalValuex - 1, minutesInDay - 1);
            const slot = `${formatMinute(startMinute)} - ${formatMinute(endMinute)}`;
            slots[slot] = [];
            currentMinute += intervalValuex;
        }
      ...
      const segmentOptions = [
        { label: 'Passo 1 minuto', value: 1 },
        { label: 'Passo 5 minuti', value: 5 },
        { label: 'Passo 15 minuti', value: 15 },
      ];
      ...
      const handleSegmentChange = (event) => {
        const newSegmentLength = event.target.value;
        // Construct the new URL with the updated segment length
        const params = new URLSearchParams(window.location.search);
        params.set('seg', newSegmentLength);
        // Optionally, keep other parameters like selectedDate and tipoRadioTV
        params.set('interval', intervalValue); // Keep the current interval if needed
        params.set('type', tipoRadioTV); // Keep the current type if needed
                ...
                <Container>
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="it">
                          <DemoContainer components={['DatePicker']}>
                            ...
                            <select id="segmentSelect" value={segmento} onChange={handleSegmentChange}>
